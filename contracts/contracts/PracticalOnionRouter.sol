...
/**
 * @title PracticalOnionRouter
 * @dev Implements actual onion routing protocol with multi-hop transaction routing
 * and encrypted layer peeling for privacy-preserving transactions
 */
contract PracticalOnionRouter is ReentrancyGuard, Pausable, Ownable {
    ...
    /**
     * @dev Register as an onion routing node
    ...
    /**
     * @dev Unregister as an onion routing node
     */
    ...
    /**
     * @dev Create onion routing packet with encrypted layers
    ...
    function createOnionPacket(
        ...
        // Verify all nodes in route are active
        uint256 totalFees = 0;
    ...
    /**
     * @dev Process onion layer by intermediate node
    ...
    function processOnionLayer(
        ...
        require(layerIndex < packet.route.length, "Invalid layer index");
        require(msg.sender == packet.route[layerIndex], "Unauthorized node");
        require(packet.currentLayer == layerIndex, "Invalid layer order");
        
        // Verify encrypted layer matches
        require(packet.encryptedLayers[layerIndex] == decryptedData, "Invalid layer data");
        
        packet.currentLayer++;
        totalProcessedLayers++;
        
        // Update node statistics
        onionNodes[msg.sender].totalProcessed++;
        onionNodes[msg.sender].totalEarned += onionNodes[msg.sender].fee;
        nodeBalances[msg.sender] += onionNodes[msg.sender].fee;
        totalFeesCollected += onionNodes[msg.sender].fee;
        
        address nextNode = address(0);
        if (layerIndex + 1 < packet.route.length) {
            nextNode = packet.route[layerIndex + 1];
        } else {
    ...
    /**
     * @dev Withdraw earned fees by node
     */
    ...
    /**
     * @dev Get onion packet information
