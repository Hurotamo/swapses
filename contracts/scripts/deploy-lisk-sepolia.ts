...
async function main() {
  console.log("🚀 Deploying to Lisk Sepolia Testnet (Chain ID: 4202)...");
  ...
  // Set up chain bridges for Lisk Sepolia testnet
  const LISK_SEPOLIA_CHAIN_ID = 4202;
  const BASE_SEPOLIA_CHAIN_ID = 84532;
  
  await walletSplitter.setChainBridge(LISK_SEPOLIA_CHAIN_ID, bridgeAddress);
  await walletSplitter.setChainBridge(BASE_SEPOLIA_CHAIN_ID, bridgeAddress);
  console.log("✅ Chain bridges configured for Lisk Sepolia testnet");
  ...
  await crossChainBridge.addValidator(validator1.address, LISK_SEPOLIA_CHAIN_ID);
  await crossChainBridge.addValidator(validator2.address, LISK_SEPOLIA_CHAIN_ID);
  await crossChainBridge.addValidator(validator3.address, LISK_SEPOLIA_CHAIN_ID);
  
  await crossChainBridge.addValidator(validator1.address, BASE_SEPOLIA_CHAIN_ID);
  await crossChainBridge.addValidator(validator2.address, BASE_SEPOLIA_CHAIN_ID);
  await crossChainBridge.addValidator(validator3.address, BASE_SEPOLIA_CHAIN_ID);
  
  console.log("✅ Validators added for both testnet chains");

  console.log("\n🎉 Lisk Sepolia Testnet Deployment Complete!");
  ...
  console.log("   Lisk Sepolia Chain ID:", LISK_SEPOLIA_CHAIN_ID);
  ...
  console.log("\n⚠️  IMPORTANT: This is deployed to Lisk Sepolia Testnet!");
  console.log("   - This is a testnet - use test tokens only");
  console.log("   - Get test LSK from Lisk Sepolia faucet");
...
main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error("❌ Lisk Sepolia deployment failed:", error);
    process.exit(1);
  }); 